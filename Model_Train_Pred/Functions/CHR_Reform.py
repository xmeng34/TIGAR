...
class CHR_Reform(object):
    ...
    def main(self):
        ...
        CHR = self.data >> select(self.data[['CHROM','POS','ID','REF','ALT','snpID']],
                                  self.data[self.sampleID])
        CHR=CHR.drop_duplicates(['snpID'],keep='first')

        if self.geno == 'vcf':
            indicate=self.data.FORMAT[0].split(":").index(self.Format)
            CHR[self.sampleID]=CHR[self.sampleID].applymap(lambda x:x.split(":")[indicate])

        CHR[self.sampleID]=CHR[self.sampleID].apply(lambda x:self.geno_reform(x,self.Format),axis=0)

        ### Calculate p_HWE (Optional) and MAF by SNPs
        temp=pd.DataFrame((CHR >> select(CHR[self.sampleID])),dtype=np.float)
        ...
        ### Dealing with NaN
        CHR[np.hstack(([self.sampleID,'MAF']))] = CHR[np.hstack(([self.sampleID,
                                                                  'MAF']))].apply(lambda x:x.fillna(2*x.MAF),axis=1)

        if self.part == 'TRAIN':
            return CHR >> mask((CHR.p_HWE>=self.p_hwe)&(CHR.MAF>=self.maf))
        elif self.part == 'PRED':
